...
- 이동: `WASD` 또는 왼쪽 조이스틱. 절반 크기 캐릭터에 맞춘 탐색 속도는 1.60 m/s, 벽 부착 이동은 1.05 m/s입니다. 캐릭터 방향은 스틱을 따라 연속으로 바뀌지만, 카메라는 진행 방향과 35도 이상 벌어진 상태가 0.30초 유지될 때만 따라옵니다.
- 카메라: 탐색 모드에서 마우스 오른쪽 드래그 또는 빈 화면 스와이프. 누르는 동안은 완전 수동이며, 손을 뗀 뒤 0.60초 동안 자동 추적이 멈춥니다.
